...
///
/// - tunnelPort: tunnel-server 在 VPS 上监听的端口
...
struct ClientConfig: Identifiable, Codable, Hashable, Sendable {
    ...
    // 自定义解码器：老数据没有 tunnelPort 时默认为 0；忽略冗余字段
    init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        self.id = try c.decode(UUID.self, forKey: .id)
        self.name = try c.decode(String.self, forKey: .name)
        self.serverId = try c.decode(UUID.self, forKey: .serverId)
        self.tunnelPort = try c.decodeIfPresent(Int.self, forKey: .tunnelPort) ?? 0
    ...
    private enum CodingKeys: String, CodingKey {
        case id, name, serverId, tunnelPort, cryptoMethod, secret, backendHost, backendPort
    }
}
