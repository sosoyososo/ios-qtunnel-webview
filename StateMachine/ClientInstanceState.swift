...
/// 实例只负责本地监听端口；每个被接受的本地连接独享一条到 tunnel-server
...
final class ClientInstanceState {
    ...
    func test(clientConfig: ClientConfig, server: Server) async -> TestResult {
        ...
        // 连接 tunnel
        let tunnel = TunnelConnection(host: server.host, port: clientConfig.tunnelPort, cipher: cipher)
    ...
    private func handleLocalConnection(_ conn: NWConnection) {
        ...
        let tunnel = TunnelConnection(host: server.host, port: cfg.tunnelPort, cipher: cipher)
